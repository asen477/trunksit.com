<div class="content">
	<div class="profile_nav">
		<ul>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=password'; ?>">修改密码</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=password&a=question'; ?>">安全问题设置</a></li>
		</ul>
	</div>
	<form id="J_pw_edit" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=password&a=edit'; ?>" method="post">
	<div class="profile_ct password_list">
		...
		<dl class="cc">
			<dt>新密码：</dt>
			<dd><input data-tips="<?php echo htmlspecialchars($pwdReg, ENT_QUOTES, 'UTF-8');?>" id="J_newPwd" type="password" name="newPwd" value="" class="input length_5" data-pwdcheck="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=password&a=checkpwd'; ?>" data-pwdstrong="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=password&a=checkpwdStrong'; ?>"><span id="J_tip_newPwd"></span></dd>
		</dl>
		<dl class="cc">
			<dt>确认密码：</dt>

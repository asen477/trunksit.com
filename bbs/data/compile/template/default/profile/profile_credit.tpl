...
<?php } else { ?>
		<li class="<?php echo htmlspecialchars($_item['current'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=extends&_left=credit&_tab=', rawurlencode($key); ?>"><?php echo htmlspecialchars($_item['title'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php 
	}
}?>
	</ul>
...
<div id="J_credit_transfer_wrap" class="wind_dialog core_pop_wrap" style="display:none;width:340px;">
	...
	<div class="pop_transfer">
		<!-- 转账 -->
		<form class="J_credit_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=credit&a=dotransfer'; ?>" method="post">
			<div class="pop_cont">
...
<div id="J_credit_change_wrap" class="wind_dialog core_pop_wrap" style="display:none;width:340px;">
	...
	<div class="pop_transform">
		<form class="J_credit_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=credit&a=doexchange'; ?>" method="post">
			<div class="pop_cont">
				<dl class="cc" style="padding-top:0;">
					<dt>积分：</dt>
...
<!--==============转换弹窗结束===============-->
<script>
var EXCHANGE = <?php echo WindSecurity::escapeEncodeJson($exchange, 'UTF-8');?>,			//积分转换数据
    TRANSFER = <?php echo WindSecurity::escapeEncodeJson($transfer, 'UTF-8');?>,			//积分转账数据
    CREDITNAME = <?php echo WindSecurity::escapeEncodeJson($creditBo->cType, 'UTF-8');?>,	//积分名
    CREDITUNIT = <?php echo WindSecurity::escapeEncodeJson($creditBo->cUnit, 'UTF-8');?>;	//积分单位

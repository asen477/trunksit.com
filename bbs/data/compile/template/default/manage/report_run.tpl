<div class="content">
	<div class="profile_nav">
		<ul>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report'; ?>">举报列表</a></li>
		</ul>
	</div>
	<div class="report_page">
		<div class="hd"><strong>筛选：</strong>
		<form action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report'; ?>" method="post">
		...
		<?php  if ($reports) { ?>
		<form class="J_form_ajax" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report&a=dealCheck'; ?>" method="post" >
			<table width="100%" class="J_check_wrap">
				...
				<tfoot>
					<tr>
						<td><label><input type="checkbox" class="J_check_all">全选</label></td>
						<td colspan="3"><button type="submit" class="btn btn_submit J_form_sub_check">标记已处理</button><button type="submit" class="btn J_form_sub_check" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report&a=delete'; ?>">忽略</button></td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
					</tr>
				</tfoot>
 ...
 echo htmlspecialchars(Pw::substrs(Pw::stripWindCode($v['content']), 20), ENT_QUOTES, 'UTF-8');
					...
					<td><?php if (!empty($v['author_username'])){?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($v['author_userid']); ?>"><?php echo htmlspecialchars($v['author_username'], ENT_QUOTES, 'UTF-8');?></a><?php }else{?>游客<?php } ?></td>
					<td><?php if (!empty($v['created_username'])){?><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($v['created_userid']); ?>" target="_blank"><?php echo htmlspecialchars($v['created_username'], ENT_QUOTES, 'UTF-8');?></a><?php }else{?>游客<?php } ?><br><?php echo htmlspecialchars(Pw::time2str($v['created_time']), ENT_QUOTES, 'UTF-8');?></td>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report&page=', rawurlencode($_page_i); 

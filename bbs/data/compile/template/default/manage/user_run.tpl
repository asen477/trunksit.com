<div class="content">
	<div class="profile_nav">
		<ul>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user'; ?>">注册审核</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user&a=email'; ?>">邮箱审核</a></li>
		</ul>
	</div>
	<div class="profile_table">
	<?php  if ($list) { ?>
	<form action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user&a=docheck'; ?>" method="post" class="J_form_ajax">
		<table width="100%" class="J_check_wrap">
			...
			<tfoot>
				<tr>
					<td><label><input type="checkbox" class="J_check_all">全选</label></td>
					<td colspan="3"><button type="submit" class="btn btn_submit J_form_sub_check">通过</button><button type="submit" class="btn J_form_sub_check" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user&a=delete'; ?>">拒绝</button></td>
					<td></td>
					<td></td>
				</tr>
			</tfoot>
			...
			<tr>
				...
				<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($item['uid']); ?>"><?php echo htmlspecialchars($item['username'], ENT_QUOTES, 'UTF-8');?></a></td>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user&page=', rawurlencode($_page_i); 

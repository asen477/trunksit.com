<div class="content">
	<div class="profile_nav">
		<ul>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content'; ?>">主题审核</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content&a=reply'; ?>">回复审核</a></li>
		</ul>
	</div>
	<div class="profile_search">
		<form method="post" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content'; ?>">
		<h2>搜索</h2>
		<table width="100%">
			<colgroup>
				<col>
	...
	<?php  if ($threadb) { ?>
	<form class="J_form_ajax" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content&a=doPassThread'; ?>" method="post">
		<h2>帖子列表</h2>
		<table width="100%" class="J_check_wrap">
			...
			<tfoot>
				<tr>
					<td><label><input type="checkbox" class="J_check_all">全选</label></td>
					<td><button type="submit" class="btn btn_submit J_form_sub_check">通过</button><button type="submit" class="btn J_form_sub_check" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content&a=doDeleteThread'; ?>">拒绝</button></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
			</tfoot>
	<?php  foreach ($threadb as $key => $value) { ?>
			<tr>
				...
				<td>
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']); ?>" target="_blank"><?php echo htmlspecialchars(Pw::stripWindCode($value['subject']), ENT_QUOTES, 'UTF-8');?></a>
					<!--p class="summary">
					他不善言辞，从不接受媒体采访，却以“J.Wong”的论坛 ID 为外界熟谙；他彻夜潜水于魅族论坛，偶尔喋喋不休，偶尔勃然大怒；他深居简出捣腾动辄万元的音乐器材，一月只出一次门理发，以至于公司五层的办公室空空荡荡。
					</p-->
				</td>
				<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>"><?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?></a></td>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content&page=', rawurlencode($_page_i); 

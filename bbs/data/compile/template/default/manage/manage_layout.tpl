...
<div class="tac"> </div>
	<div class="main_wrap">
		<div class="bread_crumb">
			<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/',''; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content'; ?>">前台管理</a>
		</div>
...
<?php  $manageLeft = Wind::getApp()->getResponse()->getData('G','manageLeft'); 
 ...
 if ($loginUser->getPermission('panel_bbs_manage')) {?>
		<li class="<?php echo htmlspecialchars($content_current, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=content'; ?>">帖子审核</a></li>
<?php }if ($loginUser->getPermission('panel_user_manage')) {?>
		<li class="<?php echo htmlspecialchars($user_current, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=user'; ?>">用户管理</a></li>
<?php } if ($loginUser->getPermission('panel_report_manage')) {?>
		<li class="<?php echo htmlspecialchars($report_current, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=report'; ?>">举报管理</a></li>
<?php } if ($loginUser->getPermission('panel_recycle_manage')) {?>
		<li class="<?php echo htmlspecialchars($recycle_current, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=recycle'; ?>">回收站</a></li>
<?php } if ($loginUser->getPermission('panel_log_manage')) {?>		
		<li class="<?php echo htmlspecialchars($manageLog_current, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=manage&c=manageLog'; ?>">管理日志</a></li>
<?php }?>
	</ul>
</div>
			</div>

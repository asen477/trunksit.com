...
<div class="wrap">
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=regist'; ?>">注册设置</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=regist&a=login'; ?>">登录设置</a></li>
			<!--<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=regist&a=guide'; ?>">新用户引导设置</a></li>-->
		</ul>
	</div>
	<div class="h_a">注册设置</div>
	<form class="J_ajaxForm" data-role="list" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=regist&a=dorun'; ?>" method="post" >
			...
			<tr>
				...
				<td>
					<ul class="single_list cc J_radio_change" id="J_register_type">
						<li><label><input type="radio" name="type" data-arr="reg2" value="1" <?php echo htmlspecialchars(Pw::ifcheck($config['type'] == 1), ENT_QUOTES, 'UTF-8');?>><span>开放注册</span></label></li>
						<li><label><input type="radio" name="type" data-arr="reg1,reg2,J_reg2_intro" value="2" <?php echo htmlspecialchars(Pw::ifcheck($config['type'] == 2), ENT_QUOTES, 'UTF-8');?>><span>邀请注册</span></label></li>
						<li><label><input type="radio" name="type" data-arr="reg3,J_reg3_intro" value="0" <?php echo htmlspecialchars(Pw::ifcheck($config['type'] == 0), ENT_QUOTES, 'UTF-8');?>><span>关闭注册</span></label></li>
					</ul>
				</td>
				<td>
				<div class="fun_tips">
...
<?php  } ?>
					...
					<td><div class="fun_tips">购买邀请码消耗的积分类型，设置消耗积分数量前往用户 &raquo; <a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=u&c=groups'; ?>" class="J_linkframe_trigger">用户组权限</a> &raquo; 基本设置。</div></td>
				</tr>
				<tr style="display:none">
					...
					<td><div class="fun_tips">此功能允许注册者通过网上支付一定费用获得邀请码，开启前请确认：1. 系统已开启<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=pay'; ?>" class="J_linkframe_trigger">网上支付</a>功能，2. 系统已开启<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=email'; ?>" class="J_linkframe_trigger">邮件发送</a>功能。</div></td>
				</tr>
				<tr style="display:none">
					<th>支付金额</th>
					<td>
		...
		<table width="100%" class="J_radio_change_items" id="reg2" style="margin-bottom:0;">
			...
			<tbody>
				...
				<!--tr>
					...
					<td><div class="fun_tips">选择“开启”，则用户在注册时需要验证手机。请确保已开启<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=mobile'; ?>" class="J_linkframe_trigger">手机服务</a>。</div></td>
				</tr-->
				<tr>
					<th>新用户注册审核</th>
					<td>
				...
				<tr>
					...
					<td><div class="fun_tips">必须先在<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=email'; ?>" class="J_linkframe_trigger">邮件设置</a>中配置邮件服务并测试成功。开启后，系统将向注册 电子邮箱 发送一封验证电子邮件以确认电子邮箱的有效性。用户收到电子邮件并激活帐号后才能拥有正常的权限。</div></td>
				</tr>
				<tr>
					<th>激活邮件标题</th>
					<td>
				...
				<tr>
					...
					<td>
						<ul class="single_list cc">
							<li><label><input type="checkbox" name="welcomeType[]" value="1" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(1, $config['welcome.type'])), ENT_QUOTES, 'UTF-8');?>><span>站内短信</span></label></li>
							<li><label><input type="checkbox" name="welcomeType[]" value="2" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(2, $config['welcome.type'])), ENT_QUOTES, 'UTF-8');?>><span>电子邮件</span></label></li>
						</ul>
					</td>
					<td><div class="fun_tips">新用户注册后系统自动发送表示欢迎的站内短信或电子邮件。<br/>如果开启了邮件激活,则只发送激活邮件,不发送欢迎邮件,必须先在<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=email'; ?>" class="J_linkframe_trigger">邮件设置</a>中配置邮件服务并测试成功。</div></td>
				</tr>
				<tr>
					<th>欢迎信息标题</th>
					<td>
				...
				<tr>
					...
					<td>
						<ul class="three_list cc">
							<li><label><input type="checkbox" value="1" name="securityPassword[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(1, $config['security.password'])), ENT_QUOTES, 'UTF-8');?>><span>小写字母</span></label></li>
							<li><label><input type="checkbox" value="2" name="securityPassword[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(2, $config['security.password'])), ENT_QUOTES, 'UTF-8');?>><span>大写字母</span></label></li>
							<li><label><input type="checkbox" value="4" name="securityPassword[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(4, $config['security.password'])), ENT_QUOTES, 'UTF-8');?>><span>数字</span></label></li>
							<li><label><input type="checkbox" value="8" name="securityPassword[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(8, $config['security.password'])), ENT_QUOTES, 'UTF-8');?>><span>符号</span></label></li>
							<li style="width:66%;"><label><input type="checkbox" value="9" name="securityPassword[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray(9, $config['security.password'])), ENT_QUOTES, 'UTF-8');?>><span>密码不能与用户名相同</span></label></li>
						</ul>
					</td>
				...
				<tr>
					...
					<td>
						<ul class="three_list cc">
							<li><label><input type="checkbox" value="location" name="activeField[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray('location', $config['active.field'])), ENT_QUOTES, 'UTF-8');?>><span>现居住地</span></label></li>
							<li><label><input type="checkbox" value="hometown" name="activeField[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray('hometown', $config['active.field'])), ENT_QUOTES, 'UTF-8');?>><span>家乡</span></label></li>
							<li><label><input type="checkbox" value="qq" name="activeField[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray('qq', $config['active.field'])), ENT_QUOTES, 'UTF-8');?>><span>QQ</span></label></li>
							<li><label><input type="checkbox" value="msn" name="activeField[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray('msn', $config['active.field'])), ENT_QUOTES, 'UTF-8');?>><span>MSN</span></label></li>
							<li><label><input type="checkbox" value="aliww" name="activeField[]" <?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray('aliww', $config['active.field'])), ENT_QUOTES, 'UTF-8');?>><span>阿里旺旺</span></label></li>
						</ul>
					</td>

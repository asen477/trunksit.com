...
<div class="wrap J_check_wrap">	
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=push'; ?>">显示中数据</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?status=1&m=design&c=push&a=status'; ?>">待审核数据</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?status=2&m=design&c=push&a=status'; ?>">推送数据</a></li>	
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=push&a=shield'; ?>">屏蔽数据</a></li>	
		</ul>
	</div>
	...
	<form method="post"  action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=push'; ?>" >
	...
	<form class="J_ajaxForm" method="post"  action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=push&a=batchshield'; ?>" >
			<?php  if ($list) { ?>
	<div class="table_list">
		<table width="100%">
			...
			<tr>
				...
				<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=push&a=doshield'; ?>" class="mr10 J_ajax_del" data-msg="确定要删除选中内容?" data-pdata="{'dataid': '<?php echo htmlspecialchars($v['data_id'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></td>
			</tr>
			<?php  } ?>
		</table>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=design&c=push'; 
...
$(function(){
	...
	function flatSet(pageid, id){
		if(pageid) {
			$.post('<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=page&a=getModuleOption'; ?>', {pageid : pageid}, function(data){

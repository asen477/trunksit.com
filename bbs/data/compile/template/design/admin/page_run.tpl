...
<div class="wrap">	
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=page'; ?>">系统页面</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=portal'; ?>">自定义页面</a></li>
		</ul>
	</div>
		...
		<table width="100%">
			...
			<tr>
				...
				<td>
					...
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?id=', rawurlencode($v['page_id']),'&m=design&c=permissions&a=page'; ?>" class="mr5">[权限管理]</a>
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?pageid=', rawurlencode($v['page_id']),'&m=design&c=module'; ?>" class="mr5">[模块管理]</a>
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=design&c=page&a=doclear'; ?>" class="mr5 J_ajax_del" data-pdata="{'id': '<?php echo htmlspecialchars($v['page_id'], ENT_QUOTES, 'UTF-8');?>'}" data-msg="您确定要清空页面上的所有模块？<br>清空后将不可恢复！">[清空模块]</a>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=design&c=page'; 

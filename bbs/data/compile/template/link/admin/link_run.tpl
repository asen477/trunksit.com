...
<div class="wrap J_check_wrap">
	<div class="nav">
	<ul class="cc">
	<li class="<?php echo htmlspecialchars($tabs['run'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link'; ?>">链接列表</a></li>
	<li class="<?php echo htmlspecialchars($tabs['editTypes'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=types'; ?>">链接分类</a></li>
	<li class="<?php echo htmlspecialchars($tabs['check'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=check'; ?>">链接审核</a></li>
	</ul>
</div>
...
<!--==============================链接列表================================-->
	<div class="mb10">
		<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=add'; ?>" class="btn J_dialog" title="添加友情链接"><span class="add"></span>添加友情链接</a>
	</div>
	<form class="J_ajaxForm" data-role="list" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=dorun'; ?>" method="post">
  ...
  foreach ($links as $v) { ?>
			<tr>
				...
				<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?lid=', rawurlencode($v['lid']),'&m=link&c=link&a=edit'; ?>" title="编辑友情链接" class="mr10 J_dialog">[编辑]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=doDelete'; ?>" class="mr10 J_ajax_del" data-pdata="{'lid': '<?php echo htmlspecialchars($v['lid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></td>
			</tr>
			<?php  } 
  } else { ?>
		...
		Wind.dialog({
			...
			onOk	: function() {
				$('form.J_ajaxForm').ajaxSubmit({
					dataType : 'json',
					url		 : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=doDelete'; ?>',

...
<!--==============================链接分类================================-->
	...
	<form method="post" class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=doTypes'; ?>" data-role="list">
  ...
  foreach ($typesList as $key => $value) { ?>
			<tr>
				...
				<td><a href="#" style="z-index:0;" class="mr10 J_copy_clipboard" data-rel="J_clipboard_copy<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>">[复制调用代码]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?typeid=', rawurlencode($value['typeid']),'&m=link&c=link'; ?>" class="mr10">[查看]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=link&c=link&a=doDeleteType'; ?>" class="mr10 J_ajax_del" data-pdata="{'typeId': '<?php echo htmlspecialchars($value['typeid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a></td>
			</tr>
			<?php  } 
  } else { ?>

...
<div class="wrap J_check_wrap">
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal'; ?>">勋章管理</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=award'; ?>">勋章颁发</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=approval'; ?>">勋章审核</a></li>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=set'; ?>">勋章设置</a></li>
		</ul>
	</div>
	<div class="mb10"><a title="添加勋章" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=add'; ?>" class="btn J_dialog"><span class="add"></span>添加勋章</a></div>
	<form class="J_ajaxForm" data-role="list" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=dorun'; ?>" method="post">
	<div class="table_list">
		...
		<?php  foreach ($medalList as $medal){
			...
			<tr>
				...
				<td><a title="编辑勋章" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?id=', rawurlencode($medal['medal_id']),'&m=medal&c=medal&a=edit'; ?>" class="mr5 J_dialog">[编辑]</a>
					<?php  if($medal['medal_type'] != 1){ ?>
					<a class="J_ajax_del" data-msg="确定要删除此勋章？" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=medal&c=medal&a=doDel'; ?>" data-pdata="{'id': '<?php echo htmlspecialchars($medal['medal_id'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a>
					<?php  } ?>
				</td>
			</tr>
		<?php  } ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=medal&c=medal'; 

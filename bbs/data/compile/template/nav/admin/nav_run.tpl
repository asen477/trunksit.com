...
<div class="nav">
  ...
  if($navType == $key){ ?>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?type=', rawurlencode($key),'&m=nav&c=nav'; ?>"><?php echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8');?></a></li>
		<?php  }else{ ?>
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?type=', rawurlencode($key),'&m=nav&c=nav'; ?>"><?php echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8');?></a></li>
		<?php  } 
  } ?>
	</ul>
</div>
...
<!--div class="mb10">
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=nav&c=nav&a=add'; ?>&type=<?php echo htmlspecialchars($navType, ENT_QUOTES, 'UTF-8');?>" class="btn J_dialog"><span class="add"></span>添加导航</a>
</div-->
<form method="post" class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=nav&c=nav&a=dorun'; ?>" data-role="list">
...
<div class="table_list">
	...
	?>
		...
		<tr>
			...
			<td>
				...
				<?php  if ($navType == 'main'){ ?>
				<a style="display:none" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?parentid=', rawurlencode($value['navid']),'&type=', rawurlencode($navType),'&m=nav&c=nav&a=add'; ?>" class="link_add J_addChild add_nav" data-id="<?php echo htmlspecialchars($value['navid'], ENT_QUOTES, 'UTF-8');?>" data-html="tbody" data-type="nav_2">添加二级导航</a>
				<?php  } ?>
			</td>
  ...
  if ($navType == 'my'){ ?>
			...
			<td>
				<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?navid=', rawurlencode($value['navid']),'&type=', rawurlencode($value['type']),'&m=nav&c=nav&a=edit'; ?>" class="mr10 J_dialog" title="导航编辑">[编辑]</a>
				<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=nav&c=nav&a=del'; ?>" class="mr10 J_ajax_del" data-pdata="{'navid': '<?php echo htmlspecialchars($value['navid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a>
			</td>
		</tr>
		</tbody>
		<?php 
		...
		?>
			<tr>
				...
				<td>
					<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?navid=', rawurlencode($childValue['navid']),'&type=', rawurlencode($childValue['type']),'&m=nav&c=nav&a=edit'; ?>" class="mr10 J_dialog" title="导航编辑">[编辑]</a><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=nav&c=nav&a=del'; ?>" class="mr10 J_ajax_del" data-pdata="{'navid': '<?php echo htmlspecialchars($childValue['navid'], ENT_QUOTES, 'UTF-8');?>'}">[删除]</a>
				</td>
			</tr>
		<?php 
			}
